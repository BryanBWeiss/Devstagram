...
<div class="md:w-6/12 p-5">
        ...
        <div class="md:w-4/12 bg-white p-6 rounded-lg shadow-xl">
             <form  method="POST" action="{{ route('login')}} ">
                @csrf
                     @if (session('mensaje'))
                     <p class=" bg-red-400 text-white my-2 rounded-lg p-2 font-bold text-center">
                        {{(session('mensaje'))}} </p>
       ...
       <div class="mb-5">
             <label for="password" class="mb-2   block uppercase text-gray-400 font-bold">
              ...
              <input id="password" name="password" type="password"
               ...
               class="border p-3 w-full rounded-lg mb-3 @error('password') border-red-500 @enderror" />
               @error('password')
               <p class="bg-red-500 text-white my-2 rounded-lg text-sm p-2 text-center">
                 {{ $message }}
                </p>
                ...
                <div class="mb-5">
                   <input type="checkbox" name="remenber"> <label class="  
                   text-gray-500 text-sm"> Mantener
                     mi sesion Activa</label> 
                </div> 
