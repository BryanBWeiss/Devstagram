...
<div class="md:w-6/12 p-5">
        ...
        <div class="md:w-4/12 bg-white p-6 rounded-lg shadow-xl">
             <form action="{{ route('register') }}" method="POST">
                @csrf
    <div class="mb-5">
            <label for="name" class="mb-2   block uppercase text-gray-400 font-bold">
             Nombre
            </label>
  ...
  <div class="mb-5">
            <label for="username" class="mb-2   block uppercase text-gray-400 font-bold">
            Username
            </label>
               <input id="username" name="username" type="text" placeholder="Tu Nombre de Usuario"
               class="border p-3 w-full rounded-lg  @error('username') border-red-500 @enderror"
               value="{{ old('username') }}"  />
            @error('username') <p class="bg-red-500 text-white my-2 rounded-lg text-sm p-2 text-center">
            {{ $message }}
            </p>
            @enderror
  </div>
  ...
  <div>
       <div class="mb-5">
            <label for="password_confirmation"
              class="mb-2   block uppercase text-gray-400 font-bold">
              Repetir Password
              </label>
              <input id="password_confirmation" name="password_confirmation"
               type="password" placeholder="Confirma Password de registro"
               class="border p-3 w-full rounded-lg mb-3 @error('password_confirmation') border-red-500 @enderror"
              {{-- se agrego mb-3 del original --}} />
